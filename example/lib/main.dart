...
class _AppState extends State<App> {
  ...
  void _addListener() {
    Chime.eventChannel.receiveBroadcastStream().listen((data) async {
      print('Chime eventChannel received data on listen');
    ...
    try {
      meetingSessionState = await Chime.createMeetingSession(
          ...
          mediaPlacementAudioFallbackUrl:
              'wss://haxrp.m1.ec1.app.chime.aws:443/calls/Test-MeetingId',
          mediaPlacementSignalingUrl:
              'wss://signal.m1.ec1.app.chime.aws/control/Test-MeetingId',
  ...
  void _handleOnVideoTileAdded(dynamic arguments) async {
    ...
    if (mapping != null) {
      print(
          '_handleOnVideoTileAdded: Already mapped. TileId=${mapping.tileId}] => ViewId=${mapping.viewId} => binding again');
    ...
    for (int viewIndex = 0; viewIndex < MAX_VIEW_COUNT; viewIndex++) {
      ...
      if (mapping.tileId == null) {
        ...
        print(
            '_handleOnVideoTileAdded: New mapping: TileId=${mapping.tileId} => ViewId=${mapping.viewId} => binding');
        await Chime.bindVideoView(mapping.viewId, mapping.tileId);
        return;
      }
    }
    ...
    print(
        'Error: _handleOnVideoTileAdded: Could not find free view. Ignoring new video tile.');
  }
  ...
  void _handleOnVideoTileRemoved(dynamic arguments) async {
    ...
    if (mapping == null) {
      print(
          'Error: _handleOnVideoTileRemoved: Could not find mapping for TileId=$tileId');
      return;
    }

    print(
        '_handleOnVideoTileRemoved: Found mapping: TileId=${mapping.tileId} => ViewId=${mapping.viewId} => unbinding');
    mapping.tileId = null;
